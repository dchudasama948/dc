...
ACCOUNT_SIZE = 100.0
RISK_PCT     = 0.10       # 10% of current balance per trade
MAX_TRADES_PER_DAY = 1    # enforce 1 trade per day max
CHART_FILE   = "pnl_dashboard.png"

# Setup-specific TP multipliers (×risk distance)
S1_TP_RR = 1.5
S2_TP_RR = 2.5
S3_TP_RR = 2.2
...
if __name__ == '__main__':
    print('\n🚀  ETHUSDT Deep Quant Backtest v2.0')
    print('    3 Setups  ·  10% Risk  ·  1 Trade/Day  ·  Full Visual Dashboard\n')

    csv_path = sys.argv[1] if len(sys.argv) > 1 else CSV_FILE

    print(f'📂  Loading: {csv_path}')
    df_raw = load_data(csv_path)
    print(f'✅  {len(df_raw):,} candles — {df_raw["timestamp"].min().date()} to {df_raw["timestamp"].max().date()}')

    print('📊  Building 57 indicators...')
    df_ind = build_indicators(df_raw)
    print(f'✅  Ready — {len(df_ind):,} candles after warmup')

    print('🔍  Scanning all 3 setups (1 trade/day limit)...\n')
